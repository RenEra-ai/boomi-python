...
class AtomServiceAsync(AtomService):
    ...
    def create_atom(self, request_body: Atom = None) -> Awaitable[Union[Atom, str, dict]]:
        return to_async(super().create_atom)(request_body)

    def get_atom(self, id_: str) -> Awaitable[Union[Atom, str, dict]]:
        return to_async(super().get_atom)(id_)

    def update_atom(
        self, id_: str, request_body: Atom = None
    ) -> Awaitable[Union[Atom, str, dict]]:
    ...
    ) -> Awaitable[Union[AtomBulkResponse, str, dict]]:
        return to_async(super().bulk_atom)(request_body)

    def query_atom(
        self, request_body: AtomQueryConfig = None
    ) -> Awaitable[Union[AtomQueryResponse, str, dict]]:
    ...
    def async_token_atom_counters(
        self, token: str
    ) -> Awaitable[Union[AtomCountersAsyncResponse, str, dict]]:
