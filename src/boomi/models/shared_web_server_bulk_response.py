...
class SharedWebServerBulkResponseResponse(BaseModel):
    """SharedWebServerBulkResponseResponse

    :param result: result, defaults to None
    :type result: SharedWebServer, optional
    ...
    def __init__(
        self,
        result: SharedWebServer = SENTINEL,
        index: int = SENTINEL,
        id_: str = SENTINEL,
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, SharedWebServer)
...
@JsonMap({})
class SharedWebServerBulkResponse(BaseModel):
    """SharedWebServerBulkResponse

    :param response: response, defaults to None
    :type response: List[SharedWebServerBulkResponseResponse], optional
    """

    def __init__(
        self, response: List[SharedWebServerBulkResponseResponse] = SENTINEL, **kwargs
    ):
        """SharedWebServerBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(
                response, SharedWebServerBulkResponseResponse
            )
        self._kwargs = kwargs
