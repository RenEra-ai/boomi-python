...
from .shared_server_information import SharedServerInformation
...
@JsonMap({})
class SharedServerInformationBulkResponse(BaseModel):
    """SharedServerInformationBulkResponse

    :param response: response, defaults to None
    :type response: List[SharedServerInformationBulkResponseResponse], optional
    """

    def __init__(
        self,
        response: List[SharedServerInformationBulkResponseResponse] = SENTINEL,
        **kwargs,
    ):
        """SharedServerInformationBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(
                response, SharedServerInformationBulkResponseResponse
            )
        self._kwargs = kwargs
