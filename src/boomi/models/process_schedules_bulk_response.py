...
class ProcessSchedulesBulkResponseResponse(BaseModel):
    """ProcessSchedulesBulkResponseResponse

    :param result: result, defaults to None
    :type result: ProcessSchedules, optional
    ...
    def __init__(
        self,
        result: ProcessSchedules = SENTINEL,
        index: int = SENTINEL,
        id_: str = SENTINEL,
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, ProcessSchedules)
...
@JsonMap({})
class ProcessSchedulesBulkResponse(BaseModel):
    """ProcessSchedulesBulkResponse

    :param response: response, defaults to None
    :type response: List[ProcessSchedulesBulkResponseResponse], optional
    """

    def __init__(
        self, response: List[ProcessSchedulesBulkResponseResponse] = SENTINEL, **kwargs
    ):
        """ProcessSchedulesBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(
                response, ProcessSchedulesBulkResponseResponse
            )
        self._kwargs = kwargs
