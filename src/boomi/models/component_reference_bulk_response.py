...
@JsonMap({})
class ComponentReferenceBulkResponse(BaseModel):
    """ComponentReferenceBulkResponse

    :param response: response, defaults to None
    :type response: List[ComponentReferenceBulkResponseResponse], optional
    """

    def __init__(
        self,
        response: List[ComponentReferenceBulkResponseResponse] = SENTINEL,
        **kwargs,
    ):
        """ComponentReferenceBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(
                response, ComponentReferenceBulkResponseResponse
            )
        self._kwargs = kwargs
