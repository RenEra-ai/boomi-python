...
class CloudBulkResponseResponse(BaseModel):
    """CloudBulkResponseResponse

    :param result: result, defaults to None
    :type result: Cloud, optional
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, Cloud)
...
@JsonMap({})
class CloudBulkResponse(BaseModel):
    """CloudBulkResponse

    :param response: response, defaults to None
    :type response: List[CloudBulkResponseResponse], optional
    """

    def __init__(self, response: List[CloudBulkResponseResponse] = SENTINEL, **kwargs):
        """CloudBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(response, CloudBulkResponseResponse)
        self._kwargs = kwargs
