...
class FolderBulkResponseResponse(BaseModel):
    """FolderBulkResponseResponse

    :param result: result, defaults to None
    :type result: Folder, optional
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, Folder)
...
@JsonMap({})
class FolderBulkResponse(BaseModel):
    """FolderBulkResponse

    :param response: response, defaults to None
    :type response: List[FolderBulkResponseResponse], optional
    """

    def __init__(self, response: List[FolderBulkResponseResponse] = SENTINEL, **kwargs):
        """FolderBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(response, FolderBulkResponseResponse)
        self._kwargs = kwargs
