...
class RoleBulkResponseResponse(BaseModel):
    """RoleBulkResponseResponse

    :param result: result, defaults to None
    :type result: Role, optional
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, Role)
...
class RoleBulkResponse(BaseModel):
    ...
    :type response: List[RoleBulkResponseResponse], optional
    """

    def __init__(self, response: List[RoleBulkResponseResponse] = SENTINEL, **kwargs):
        """RoleBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(response, RoleBulkResponseResponse)
        self._kwargs = kwargs
