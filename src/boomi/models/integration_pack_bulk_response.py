...
from .integration_pack import IntegrationPack
...
class IntegrationPackBulkResponseResponse(BaseModel):
    """IntegrationPackBulkResponseResponse

    :param result: result, defaults to None
    :type result: IntegrationPack, optional
    ...
    def __init__(
        self,
        result: IntegrationPack = SENTINEL,
        index: int = SENTINEL,
        id_: str = SENTINEL,
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, IntegrationPack)
...
@JsonMap({})
class IntegrationPackBulkResponse(BaseModel):
    """IntegrationPackBulkResponse

    :param response: response, defaults to None
    :type response: List[IntegrationPackBulkResponseResponse], optional
    """

    def __init__(
        self, response: List[IntegrationPackBulkResponseResponse] = SENTINEL, **kwargs
    ):
        """IntegrationPackBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(
                response, IntegrationPackBulkResponseResponse
            )
        self._kwargs = kwargs
