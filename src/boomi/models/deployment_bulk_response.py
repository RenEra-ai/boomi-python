...
class DeploymentBulkResponseResponse(BaseModel):
    """DeploymentBulkResponseResponse

    :param result: result, defaults to None
    :type result: Deployment, optional
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, Deployment)
...
@JsonMap({})
class DeploymentBulkResponse(BaseModel):
    """DeploymentBulkResponse

    :param response: response, defaults to None
    :type response: List[DeploymentBulkResponseResponse], optional
    """

    def __init__(
        self, response: List[DeploymentBulkResponseResponse] = SENTINEL, **kwargs
    ):
        """DeploymentBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(response, DeploymentBulkResponseResponse)
        self._kwargs = kwargs
