...
@JsonMap({})
class SharedCommunicationChannelComponentBulkResponse(BaseModel):
    """SharedCommunicationChannelComponentBulkResponse

    :param response: response, defaults to None
    :type response: List[SharedCommunicationChannelComponentBulkResponseResponse], optional
    """
        ...
        **kwargs,
        ...
        if response is not SENTINEL:
            self.response = self._define_list(
                response, SharedCommunicationChannelComponentBulkResponseResponse
            )
        self._kwargs = kwargs
