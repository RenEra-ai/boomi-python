...
class MergeRequestBulkResponseResponse(BaseModel):
    """MergeRequestBulkResponseResponse

    :param result: result, defaults to None
    :type result: MergeRequest, optional
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, MergeRequest)
...
@JsonMap({})
class MergeRequestBulkResponse(BaseModel):
    """MergeRequestBulkResponse

    :param response: response, defaults to None
    :type response: List[MergeRequestBulkResponseResponse], optional
    """

    def __init__(
        self, response: List[MergeRequestBulkResponseResponse] = SENTINEL, **kwargs
    ):
        """MergeRequestBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(
                response, MergeRequestBulkResponseResponse
            )
        self._kwargs = kwargs
