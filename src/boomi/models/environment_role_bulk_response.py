...
class EnvironmentRoleBulkResponseResponse(BaseModel):
    """EnvironmentRoleBulkResponseResponse

    :param result: result, defaults to None
    :type result: EnvironmentRole, optional
    ...
    def __init__(
        self,
        result: EnvironmentRole = SENTINEL,
        index: int = SENTINEL,
        id_: str = SENTINEL,
        ...
        **kwargs,
        ...
        if result is not SENTINEL:
            self.result = self._define_object(result, EnvironmentRole)
...
@JsonMap({})
class EnvironmentRoleBulkResponse(BaseModel):
    """EnvironmentRoleBulkResponse

    :param response: response, defaults to None
    :type response: List[EnvironmentRoleBulkResponseResponse], optional
    """

    def __init__(
        self, response: List[EnvironmentRoleBulkResponseResponse] = SENTINEL, **kwargs
    ):
        """EnvironmentRoleBulkResponse
        ...
        if response is not SENTINEL:
            self.response = self._define_list(
                response, EnvironmentRoleBulkResponseResponse
            )
        self._kwargs = kwargs
